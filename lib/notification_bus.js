...
initCreator = function(that){
  ...
  that.subClient.on('taskMessage', function(channel, message){
    var id;
    id = message;
    that.emit('notification_received', id);
  });
};
...
exports.initialize = function(options, callback) {
  ...
  NotificationBus = function(options, callback){
    ...
    var functions = [
      ...
      function(cb) {that.dataClient = dataStore.initialize(options, cb).messageStore;}
    ];
  ...
  NotificationBus.prototype.sendNotification = function(channel, id, message, status, callback) {
    ...
    this.dataClient.create(hash, id, msgString, function(err, reply){
      if (err){
        callback(new Error("Error sending message: " + err));
      } else {
        that.pubClient.publish(channel, pubMessage, function() {
  ...
  NotificationBus.prototype.processNotification = function(channel, message, callback) {
    ...
    if (!message.id || !message.status) {
      // This shouldn't happen - means malformed message
      that.emit('error', new Error("Malformed message expected {id: 'someId', status: 'someStatus'}.  Got " + message));
      return;
    }
    ...
    that.dataClient.process(hash, id, function(err, result) {
