    ...
    BackgroundTask.prototype.acceptTask = function(id, callback){
      ...
      newCallback = function(reply){
        ...
        if (reply[0] instanceof Error){
          console.log("NBT:  Reply 0 error");
          console.log(util.inspect(reply[[0]]));
          if (reply[0].message === "DB doesn't recognize message"){
            result = new Error('Task not in database, do not accept');
            callback(result);
            return;
          }
        } else if (reply[1] instanceof Error) {
          console.log("NBT:  Reply 0 error");
          console.log(util.inspect(reply[[1]]));
          result = new Error('Second task was an error: ' + reply[1]);
          callback(result);
          return;
        }

        if (reply[0].taskDetails){
          callback(reply[0].taskDetails);
        } else {
          callback(reply[0]);
        }
      };
