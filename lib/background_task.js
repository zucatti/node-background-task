      ...
      newCallback = function(reply){
        ...
        if (reply instanceof Error){
          if (reply.message === "DB doesn't recognize message"){
            result = new Error('Task not in database, do not accept');
            callback(result);
            return;
          }
        }
        ...
        that.msgBus.acceptMessage(id, function(msgBusReply) {
          if (msgBusReply instanceof Error) {
            newCallback(msgBusReply)
          } else {
            that.progressBus.acceptMessage(id, function(progressBusReply) {
              if (progressBusReply instanceof Error) {
                newCallback(progressBusReply);
              } else {
                newCallback(msgBusReply);
              }
            });
        ...
        startTheTask = function(){
            that.taskLimit.startTask(msg, function(tasks){
                ...
                cb = function(reply){
                    ...
                    that.msgBus.cleanup(tid,function() {});
                    that.progressBus.cleanup(tid,function() {});
