...
exports.initialize = function(options, callback) {
  ...
  var messageStoreCreate = function(channel, id, message, callback) {
    var that = this;
    // TODO Precondition checks

    that.redis.setHashKey(channel, id, message, function(err, result) {
      utils.processResult(err, result, callback);
    });
  };

  var messageStoreProcess = function(channel, id, callback) {
    var that = this;
    // TODO Precondition checks

    that.redis.getAndDeleteHashKey(channel, id, function(err, result) {
      utils.processResult(err, result, callback);
    });
  };
